...
- Graceful degradation: never throw when partial results are available. Use `warnings[]` array. This includes the frame-only tools (`get_frames`/`get_frame_at`/`get_frame_burst`): a zero-frame outcome — extraction failure OR everything filtered as black/duplicate — returns `frameCount: 0` with the accumulated warnings, never a thrown `UserError` (issue #26). Keep the input-validation throws (`getAdapter`/bad timestamp) — only the extraction outcome degrades. Wrap `extractFrameAt`/`extractFrameBurst` calls in `try/catch`; they throw a raw ffmpeg `Error` (which also leaks the command line) unlike `extractKeyFrames`, which already degrades to `[]`.
