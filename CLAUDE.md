...
- yt-dlp platform URLs (single-video pages only; playlists/channels rejected) route through `YtDlpAdapter`. `findYtDlp()` + `ytdlpCookieArgs()` are shared via `src/utils/ytdlp.ts`. Missing yt-dlp degrades to install-hint warnings, never throws; `downloadVideo` must return `null`, never reject (the pipeline calls it without catch). Native captions preferred (uploaded > auto-generated with rolling-window collapse); no captions → `[]` → Whisper fallback.
